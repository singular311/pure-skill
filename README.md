...
Типова папка з тайтлами — `D:\укр_webp`, тому параметр `source` більше не потрібен. Для іншої папки її можна передати першим аргументом. Для фактичного завантаження виконай `python .\publish.py --publish`. Скрипт завантажує лише нові або змінені WebP, оновлює каталог і не видаляє файли з R2. Потім він сам створює Git-коміт та робить `git push`; цей push запускає Pages-деплой у GitHub Actions. Якщо каталог не змінився, коміт і деплой не виконуються. Якщо `git push` завершується помилкою, стан локальної публікації не зберігається, тому наступний запуск безпечно повторить синхронізацію.

На Windows скрипт сам знаходить `wrangler.cmd`, тому не потрібно передавати його шлях вручну. Якщо Wrangler встановлено поза `PATH`, передай його через `--wrangler 'C:\\шлях\\до\\wrangler.cmd'`.
